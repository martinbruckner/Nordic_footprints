...
for(year in years){
  ...
  for(region in regions){
    ...
    for(ee in 1:ncol(extension)){
      ...
      # colnames(FP) <- IO.codes$Product.Name
      colnames(FP) <- rep(Product.codes$Food, nrreg)
      FP <- agg(FP)
      FP <- t(FP)
      
      # Aggregate Countries of origin
      colnames(FP) <- IO.codes$Country.Code
      FP <- agg(FP)
      
      FP <- melt(FP)
      colnames(FP) <- c("product","source","value")
      ...
      FP_final <- rbind(FP_final, FP)
      
    }
  }
  fwrite(FP_final, file=paste0("./results/Nordic_Footprints_",year,".csv"), row.names = FALSE)
}
